...
export default function Displays() {
	...
	const handleResolutionChange = (monitorId: number, resolution: string) => {
		...
		if (monitor) {
			...
			const newRefreshRate = availableRatesForResolution.includes(
				currentRefreshRate,
			)
				? currentRefreshRate
	...
	const displayMonitors =
		loading && cachedMonitors.length > 0 ? cachedMonitors : monitors;
							...
							<CardContent>
								<div className="grid grid-cols-2 gap-4 text-sm">
									<div className="space-y-2">
										...
										<Select
											value={
												monitorSettings[monitor.id]?.resolution ||
												`${monitor.width}x${monitor.height}`
											}
											onValueChange={(value) =>
												handleResolutionChange(monitor.id, value)
											}
										>
									...
									<div className="space-y-2">
										...
										<Select
											value={
												monitorSettings[monitor.id]?.refreshRate ||
												monitor.refresh_rate.toFixed(2)
											}
											onValueChange={(value) =>
												handleRefreshRateChange(monitor.id, value)
											}
										>
											<SelectTrigger className="w-full">
												<SelectValue />
